 ...
 * de `btv-cli` (precisam de `btv-sidecar`). Rótulo/nav dizem "RAG", mas
 * a recuperação é léxica (`recall.py`, ADR 0013), não semântica — a tela
 * carrega essa tensão honesta, não some com ela.
 */
import { fetchJson } from './client'

/** Espelha `btv_proto::memory::MemorySummary` — sem coluna de tendência
...
/** Espelha `btv_proto::memory::MemoryMatch`. */
