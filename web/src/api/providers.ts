 ...
 * /api/providers`, `btv-server` — reusa `Gateway::from_env().available()`,
 * a MESMA leitura de env vars que uma sessão `btv run`/`chat` real usaria;
 ...
 * desta onda: `btv_llm::FallbackChain` é código morto (`Gateway::generate`
 * itera os providers configurados direto, nunca consulta
 * `FallbackChain::next_after` — confirmado lendo o código antes de expor
 * qualquer mutação) e, mesmo se fosse consultada, o `btv dashboard` é um
