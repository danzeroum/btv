//! Configuração de passos do `/verify` por projeto (`btv.toml` na raiz).
...
/// Passos default quando não há `btv.toml`: espelha o job `rust` do CI
...
mod tests {
    ...
    fn arquivo_ausente_retorna_none() {
        let result = load_config(Path::new("/tmp/este-arquivo-btv-toml-nao-existe.toml"));
    ...
    fn toml_invalido_retorna_erro() {
        ...
        let path = dir.path().join("btv.toml");
