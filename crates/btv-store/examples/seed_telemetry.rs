//! Semeia um evento real de telemetria num `.btv/telemetry.db`, usando o
//! mesmo `btv_store::Telemetry` que o restante do produto usa — não é um
//! hack de SQL cru. Existe para testes de integração cross-process (ex.:
//! o e2e de `web/` que sobe um `btv dashboard` real e confirma que a
//! tela de Telemetria reflete um evento gravado por fora).
//!
//! Uso: cargo run -p btv-store --example seed_telemetry -- <db_path> <nome> <session_id> [props_json] [ts]
