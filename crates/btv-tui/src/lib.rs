...
/// esta crate não conhece `btv-tools`; quem converte é o `btv-cli`).
...
pub fn render(frame: &mut Frame, state: &TuiState) {
    ...
    let transcript = Paragraph::new(lines.into_iter().skip(skip).collect::<Vec<_>>())
        ...
        .block(Block::default().borders(Borders::ALL).title(" btv "));
...
mod tests {
    ...
    fn transcript_input_e_status_aparecem() {
        ...
        assert!(screen.contains("btv ▸ Pronto, teste corrigido."));
    ...
    fn streaming_aparece_e_finish_turn_move_para_o_transcript() {
        ...
        assert!(draw(&state).contains("btv ▸ Analisando"));

        state.streaming.push_str(" o arquivo...");
        state.finish_turn();
        assert!(state.streaming.is_empty());
        assert_eq!(state.items.len(), 1);
        assert!(draw(&state).contains("btv ▸ Analisando o arquivo..."));
    }
