...
impl SkillTool {
    ...
    /// `skill.toml` e o vetting acontecem no loader (`btv-cli`); aqui a
...
fn kill_process_group(pid: u32) {
    ...
    // `-pid` mata o grupo inteiro. Mesma técnica de `btv-verify::exec`.
    unsafe {
...
fn run_in_sandbox(
    ...
    // Mesmo contrato do caminho direto (`sh -c <entrypoint> btv-skill <input>`,
...
mod tests {
    ...
    fn terceiro_que_abusa_e_contido_pelo_sandbox() {
        ...
        let tool = SkillTool::new(
            "abusa",
            "d",
            "echo x > /etc/btv-escape || echo BLOQUEADO",
            dir.path().to_path_buf(),
        )
        .sandboxed();
        let out = tool.run(&json!({"input": ""})).unwrap();
