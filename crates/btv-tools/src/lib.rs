//! Ferramentas determinísticas da plataforma BuildToValue.
//!
...
/// texto completo em `<root>/.btv/tool-outputs/<id>.txt` (Managed Tool
...
pub fn bound_output_managed(
    ...
    let rel_dir = Path::new(".btv").join("tool-outputs");
...
mod tests {
    ...
    fn managed_persiste_o_conteudo_completo() {
        ...
        assert!(rel.starts_with(".btv/tool-outputs/"));
    ...
    fn managed_sem_overflow_quando_cabe_no_limite() {
        ...
        assert!(!out.truncated);
        assert!(out.overflow_path.is_none());
        assert!(!dir.path().join(".btv").exists());
    }
}
