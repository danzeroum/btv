//! Sobe o sidecar Python (`btv_promptforge`) com degradação graciosa:
...
//!
//! Localização do workspace Python: `BTV_PYTHON_DIR` se definida, senão
...
use btv_sidecar::{SidecarClient, SidecarSupervisor};
...
pub async fn try_start() -> Option<(SidecarSupervisor, SidecarClient)> {
    ...
    let socket = std::env::temp_dir().join(format!("btv-sidecar-{}.sock", std::process::id()));
...
pub fn advisory(report: &btv_proto::promptforge::LintReport) -> Option<String> {
    if report.score >= 0.9 {
        return None;
    }
