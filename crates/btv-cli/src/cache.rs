...
//! `btv_schemas::request_hash`, o mesmo contrato com paridade
...
use btv_store::{PromptCache, Telemetry};
...
impl<G: Generator> CachedGenerator<G> {
    ...
    fn cache_key(req: &GenerateRequest) -> Result<String, btv_schemas::CacheKeyError> {
        ...
        btv_schemas::request_hash(&envelope, &temperature)
    }
}
...
mod tests {
    use super::*;
    use btv_llm::chat::{ChatMessage, ContentBlock, StopReason, Usage};
