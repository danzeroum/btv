...
//! `btv-sidecar`, que `btv-server` não tem (regra de posicionamento de
//! rota da fase), por isso mora aqui e é `.merge()`ado no router do agente
//! web (`web_agent::merged_router`'s `extra`), não em `btv-server`.
//!
...
use btv_sidecar::{SidecarError, SidecarService};
...
/// (`btv_proto::promptforge::GeneratorInfo` ganha `Serialize` via
/// `btv-proto/build.rs`, mesma técnica do `SquadEvent` da Onda 4) — sem
/// DTO espelho.
...
pub fn default_sidecar_service(root: &std::path::Path) -> Arc<SidecarService> {
    ...
    let socket = root.join(".btv").join("promptforge.sock");
...
mod tests {
    ...
    async fn sidecar_indisponivel_devolve_503_explicito() {
        ...
        let service = Arc::new(btv_sidecar::SidecarService::new(
