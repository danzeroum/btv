...
//! (`.btv/btv.db` na raiz do workspace).

use btv_core::LoopEvent;
use btv_schemas::ledger::{LedgerEntry, OverrideMark};
...
impl Session {
    pub fn open(root: &Path, task: &str, model: &str) -> anyhow::Result<Self> {
        let dir = root.join(".btv");
        std::fs::create_dir_all(&dir)?;
        let store = LedgerStore::open(dir.join("btv.db").to_str().unwrap_or(".btv/btv.db"))?;
        let id = format!(
            "s{:x}",
...
fn append_entry_impl(
    ...
    let dir = root.join(".btv");
    std::fs::create_dir_all(&dir)?;
    let mut store = LedgerStore::open(dir.join("btv.db").to_str().unwrap_or(".btv/btv.db"))?;
...
/// Registra uma entrada avulsa no MESMO ledger (`.btv/btv.db`), fora do
