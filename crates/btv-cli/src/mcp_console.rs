...
//!
//! Mora aqui (não em `btv-server`) porque precisa de `btv-tools`+
//! `btv-core` — regra de posicionamento de rota da fase.

use axum::extract::State;
use axum::response::{IntoResponse, Json, Response};
use axum::routing::get;
use axum::Router;
use btv_core::{Decision, BUILD, PLAN};
use btv_tools::mcp::list_tools_blocking;
use serde::Serialize;
...
mod tests {
    ...
    /// `btv_mcp_fixture` (Fase 6 Onda 4) é um bin de `btv-tools`, não
    ...
    /// incremental, já compilado pelos testes de `btv-tools`) e apontamos
    ...
    fn mcp_fixture_bin() -> std::path::PathBuf {
        ...
        let status = std::process::Command::new("cargo")
            .args(["build", "-p", "btv-tools", "--bin", "btv_mcp_fixture"])
            .current_dir(&repo_root)
            .status()
            .expect("cargo build do btv_mcp_fixture");
        assert!(status.success(), "build do btv_mcp_fixture falhou");
        repo_root
            .join("target")
            .join("debug")
            .join("btv_mcp_fixture")
    }

    fn write_mcp_toml(root: &std::path::Path, fixture_bin: &std::path::Path) {
        std::fs::create_dir_all(root.join(".btv")).unwrap();
        std::fs::write(
            root.join(".btv").join("mcp.toml"),
            format!(
                "[[server]]\nid = \"vivo\"\ncommand = \"{}\"\nargs = []\n\n\
    ...
    async fn console_mcp_mostra_status_real_e_preview_de_politica_com_override() {
        ...
        let mut rule_store =
            btv_store::RuleStore::open(dir.path().join(".btv").join("rules.db").to_str().unwrap())
                .unwrap();
        rule_store
            .set(
                "build",
