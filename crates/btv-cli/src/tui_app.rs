//! Comando `btv tui`: chat com a interface ratatui.
//!
...
use anyhow::Result;
use btv_core::{LoopEvent, PermissionResolver};
use btv_llm::chat::ChatMessage;
use btv_llm::Generator;
use btv_tools::DiffLine;
use btv_tui::{DiffKind, Item, PermissionPrompt, TuiState};
...
pub async fn run_tui<G: Generator + Send + Sync + 'static>(
    ...
    let quit = tokio::task::block_in_place(|| -> Result<()> {
        loop {
            ...
            terminal.draw(|f| btv_tui::render(f, &state))?;
