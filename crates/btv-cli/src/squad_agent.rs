...
//! serde direto no tipo gerado pelo proto (`btv_proto::squad::SquadEvent`
//! já deriva `Serialize`, ver `btv-proto/build.rs`). O gate HITL troca o
...
/// limit/cache (mesmo `core_generate` do `btv squad` CLI);
...
/// `CoreBackend` roteirizado (e2e sem API key, `BTV_SCRIPTED=1`, mesmo
    ...
    B: CoreBackend,
{
    ...
    std::fs::create_dir_all(&btv_dir).map_err(|e| e.to_string())?;
    ...
    let core_sock = btv_dir.join("squad-pool-core.sock");
    ...
    let tool_permissions = (btv_core::BUILD.permissions)();
...
/// gera o `task_id`, escolhe o backend (roteirizado sob `BTV_SCRIPTED`,
...
pub(crate) fn start_squad_task(
    ...
    let handle = if std::env::var_os("BTV_SCRIPTED").is_some() {
...
/// Modelo do squad — configurável via `BTV_SQUAD_MODEL` porque o pool é
/// construído **uma vez só**, na subida do `btv dashboard` (capacidade 1,
...
fn squad_model() -> String {
    std::env::var("BTV_SQUAD_MODEL").unwrap_or_else(|_| "claude-sonnet-5".into())
}
...
pub fn default_squad_pool(root: &std::path::Path) -> Arc<SquadPool> {
    ...
    let socket_dir = root.join(".btv").join("squad-pool");
    let core_sock = root.join(".btv").join("squad-pool-core.sock");
...
pub fn default_hub() -> SquadHub {
    let timeout_secs = std::env::var("BTV_HITL_TIMEOUT_SECS")
        .ok()
...
mod tests {
    ...
    #[test]
    fn squad_model_le_btv_squad_model_com_fallback_pro_claude_sonnet_5() {
        std::env::remove_var("BTV_SQUAD_MODEL");
        assert_eq!(squad_model(), "claude-sonnet-5");

        std::env::set_var("BTV_SQUAD_MODEL", "deepseek-chat");
    ...
    /// API key, `BTV_SCRIPTED=1`): `POST /api/squad/run` dispara a
    ...
    async fn run_squad_via_http_com_gate_hitl_real_e_ledger() {
        ...
        // O consenso ficou registrado no MESMO ledger (.btv/btv.db) que
