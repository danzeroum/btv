...
//! `btv_core::LoopEvent`, rota SSE genérica por sessão, ponte de permissão
...
//! `{error, code}`.
//!
//! Mora em `btv-cli` (não em `btv-core`, que continua UI-agnóstico, nem
//! em `btv-server`, que continua sem depender de `btv-core`/`btv-tools`
//! — o router daqui é `.merge()`ado ao `btv_server::router()` existente).
...
use btv_llm::scripted::ScriptedGenerator;
...
pub enum SessionEvent {
    ...
    /// cadeia de hash que `.btv/btv.db` guarda) — nunca um número
    /// fabricado, mesmo no modo roteirizado (`BTV_SCRIPTED=1`).
    Done {
        ledger_verified: u64,
    },
    Error {
...
pub struct SessionTaskSpec {
    ...
    pub system: String,
    pub task: String,
    /// Raiz do workspace — abre o MESMO ledger (`.btv/btv.db`) que o CLI
...
/// Grava no MESMO ledger que `btv run` usa — inclusive no modo
...
/// `continue_run`/`persist_new` do CLI (`btv run`) para uma mensagem web —
...
fn spawn_message_task(
    ...
    tokio::task::spawn_blocking(move || {
        ...
        let outcome: anyhow::Result<(usize, crate::session::Session)> = (|| {
            ...
            durable
                .messages
                .push(btv_llm::chat::ChatMessage::user_text(&message));
            let mut resolver = WebPermissionResolver {
                hub: hub.clone(),
                session_id: session_id.clone(),
            };
...
async fn send_message_handler(
    ...
    if std::env::var_os("BTV_SCRIPTED").is_some() {
        let tools = crate::skills::build_registry(&root);
        ...
        spawn_session_task(
            ...
            SessionTaskSpec {
                ...
                system: "modo roteirizado (BTV_SCRIPTED=1) — sem provider real".into(),
                task: body.message,
                root,
            },
        );
...
/// ledger (`.btv/`), arquivo próprio (`rules.db`).
fn rules_db_path(root: &std::path::Path) -> std::path::PathBuf {
    root.join(".btv").join("rules.db")
}

fn open_rule_store(root: &std::path::Path) -> anyhow::Result<btv_store::RuleStore> {
    let path = rules_db_path(root);
    if let Some(dir) = path.parent() {
        std::fs::create_dir_all(dir)?;
    }
    Ok(btv_store::RuleStore::open(
        path.to_str().unwrap_or(".btv/rules.db"),
    )?)
}

fn rule_record_to_core(record: btv_store::RuleRecord) -> Rule {
    Rule {
        tool: record.tool,
        scope_prefix: record.scope_prefix,
        decision: match record.decision {
            btv_store::RuleDecision::Allow => Decision::Allow,
            btv_store::RuleDecision::Ask => Decision::Ask,
            btv_store::RuleDecision::Deny => Decision::Deny,
        },
    }
}
...
fn decision_from_wire(s: &str) -> Option<btv_store::RuleDecision> {
    match s {
        "allow" => Some(btv_store::RuleDecision::Allow),
        "ask" => Some(btv_store::RuleDecision::Ask),
        "deny" => Some(btv_store::RuleDecision::Deny),
        _ => None,
    }
}
...
async fn set_rule_handler(Json(body): Json<SetRuleBody>) -> Response {
    ...
    let result: anyhow::Result<btv_store::RuleRecord> = (|| {
...
/// verdade é `btv_core::{BUILD,PLAN}`, nunca uma cópia fabricada em TS.
...
fn scripted_turns_for(message: &str) -> Vec<btv_llm::chat::AssistantTurn> {
    use btv_llm::chat::{AssistantTurn, ContentBlock, StopReason, Usage};
    vec![
        AssistantTurn {
...
/// ao `btv_server::router()` existente. Ver `merged_router` para a
...
/// Compõe o router do agente web com o `btv_server::router()` existente,
...
pub async fn serve_with_agent(
    ...
    prompt_library: std::sync::Arc<std::sync::Mutex<btv_store::PromptLibrary>>,
    ledger: std::sync::Arc<std::sync::Mutex<btv_store::LedgerStore>>,
...
pub fn default_hub() -> SessionHub {
    let max_sessions = std::env::var("BTV_MAX_SESSIONS")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or(8);
    let timeout_secs = std::env::var("BTV_PERMISSION_TIMEOUT_SECS")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or(300);
...
mod tests {
    ...
    /// `.btv/`) precisam rodar mutuamente exclusivos entre si, incluindo
    ...
    /// (`BTV_SCRIPTED=1`, sem API key) dispara o mesmo `bash` real, a
    ...
    async fn post_message_real_dispara_bash_via_modo_roteirizado() {
        ...
        // independente do MESMO `.btv/btv.db` — nunca um número
    ...
    /// (`btv_core::{BUILD,PLAN}`) — nunca os valores fabricados que o mock
