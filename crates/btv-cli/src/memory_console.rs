...
//! o `MemoryService` (`btv-sidecar`, ADR 0022) — precisa de
...
use axum::Router;
use btv_sidecar::{MemoryService, SidecarError};
...
/// mesma resolução relativa que `btv_squad.server`'s `SquadServicer` já
...
pub fn default_memory_service(root: &std::path::Path) -> Arc<MemoryService> {
    ...
    let socket = root.join(".btv").join("memory.sock");
...
mod tests {
    ...
    async fn recall_e_list_via_http_batem_com_o_corpus_semeado_por_fora() {
        ...
        let service = Arc::new(btv_sidecar::MemoryService::new(
            py_dir,
