...
//! para o processo de longa duração do `btv dashboard`. O CLI de
...
/// Pool pequeno de processos `btv_squad.server` — cada slot é um
...
mod tests {
    ...
    async fn sidecar_service_reusa_processo_e_se_recupera_de_kill() {
        ...
        let socket =
            std::env::temp_dir().join(format!("btv-sidecar-service-{}.sock", std::process::id()));
    ...
    async fn memory_service_reusa_processo_e_recall_list_batem_com_o_corpus_real() {
        ...
        let socket =
            std::env::temp_dir().join(format!("btv-memory-service-{}.sock", std::process::id()));
    ...
    async fn spawn_test_core(prefix: &str) -> (tokio::task::JoinHandle<()>, PathBuf) {
        let core_sock =
            std::env::temp_dir().join(format!("btv-{prefix}-core-{}.sock", std::process::id()));
    ...
    async fn squad_pool_capacidade_dois_usa_dois_processos_distintos_concorrentes() {
        ...
        let socket_dir = std::env::temp_dir().join(format!("btv-pool2-{}", std::process::id()));
    ...
    async fn squad_pool_capacidade_um_detecta_queda_e_sobe_processo_novo() {
        ...
        let socket_dir = std::env::temp_dir().join(format!("btv-pool1-{}", std::process::id()));
        std::fs::create_dir_all(&socket_dir).unwrap();
    ...
    impl crate::CoreBackend for NoopCore {
        async fn generate(
            &self,
            _req: &btv_proto::llm::LlmRequest,
        ) -> Result<(String, btv_proto::llm::Usage), String> {
            Ok((
                "{}".into(),
                btv_proto::llm::Usage {
                    input_tokens: 0,
        ...
        async fn request_permission(&self, _req: &btv_proto::core::PermissionRequest) -> bool {
            true
        }
        async fn run_tool(&self, _call: &btv_proto::core::ToolCall) -> btv_proto::core::ToolResult {
            btv_proto::core::ToolResult {
