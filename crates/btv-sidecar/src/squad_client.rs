//! Cliente e supervisor do sidecar Python `btv_squad.server` (Onda 4d):
//! `SquadService.ExecuteTask` devolve um stream de `SquadEvent` que o
//! `btv squad` renderiza ao vivo. O supervisor spawna o processo com
...
use btv_proto::squad::squad_service_client::SquadServiceClient;
use btv_proto::squad::{squad_event, HealthRequest, SquadEvent, SquadTask};
...
impl SquadSupervisor {
    /// Spawna `uv run python -m btv_squad.server --socket <squad> --core-socket <core>`
    ...
    pub fn spawn(
        ...
        cmd.args(["run", "python", "-m", "btv_squad.server", "--socket"])
            .arg(&socket_path)
            .arg("--core-socket")
            .arg(core_socket)
            .arg("--model")
...
impl Drop for SquadSupervisor {
    ...
    /// o Python como filho, isso deixava `btv_squad.server` **órfão
