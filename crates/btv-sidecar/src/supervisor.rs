//! Sobe e supervisiona o sidecar Python (`btv_promptforge.server`) e
...
impl SidecarSupervisor {
    /// Spawna `uv run python -m btv_promptforge.server --socket <path>`
    ...
    pub fn spawn(python_workspace_dir: &Path, socket_path: PathBuf) -> Result<Self, SidecarError> {
        ...
        cmd.args(["run", "python", "-m", "btv_promptforge.server", "--socket"])
            .arg(&socket_path)
...
mod tests {
    ...
    async fn processo_inexistente_falha_rapido_com_unavailable() {
        ...
        // "uv" existe no ambiente de dev, mas sem o pacote btv_promptforge
