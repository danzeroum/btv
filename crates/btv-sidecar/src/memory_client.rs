//! Cliente e supervisor do sidecar Python `btv_squad.memory_server`
...
//! precisa de `--core-socket`: ao contrário do `btv_squad.server`, este
//! sidecar não chama de volta o Rust, só lê o que o orquestrador já gravou
//! em disco).

use crate::client::{socket_ready, SidecarError};
use btv_proto::memory::memory_service_client::MemoryServiceClient;
use btv_proto::memory::{HealthRequest, ListRequest, ListResponse, RecallRequest, RecallResponse};
...
impl MemorySupervisor {
    /// Spawna `uv run python -m btv_squad.memory_server --socket <path>
    /// [--memory-dir <dir>]` no diretório do workspace Python.
    ///
    /// `memory_dir: None` é a escolha certa em produção: `btv_squad.
    /// server`'s `SquadServicer` (que é quem de fato ESCREVE a memória, via
    /// `remember_decision`) nunca recebe `--memory-dir` hoje — cai no
    /// default de `AgentMemorySystem()` (`.btv/squad-memory` relativo ao
    ...
    pub fn spawn(
        ...
        cmd.args(["run", "python", "-m", "btv_squad.memory_server", "--socket"])
            .arg(&socket_path);
