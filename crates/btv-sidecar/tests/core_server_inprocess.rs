...
use btv_core::{Decision, PermissionEngine, Rule};
use btv_proto::core::core_service_client::CoreServiceClient;
use btv_proto::core::{PermissionRequest, ToolCall, ToolResult};
use btv_proto::llm::{llm_chunk, LlmRequest, Usage};
use btv_sidecar::{serve_core, CoreBackend};
...
impl CoreBackend for BackendWithTools {
    ...
    async fn run_tool(&self, call: &ToolCall) -> ToolResult {
        // Mesma lógica de `btv-cli::squad::core_run_tool`, sem depender de
        // `btv-cli` (evitaria um ciclo de dependência de teste) — isolado
...
async fn core_service_responde_generate_em_processo() {
    let sock = std::env::temp_dir().join(format!("btv-core-inproc-{}.sock", std::process::id()));
...
async fn run_tool_executa_de_verdade_e_arquivo_aparece_no_disco() {
    ...
    let sock =
        std::env::temp_dir().join(format!("btv-core-runtool-ok-{}.sock", std::process::id()));
    let core_task = tokio::spawn(serve_core(backend, sock.clone()));
    wait_for_socket(&sock).await;
    let mut client = connect_client(&sock).await;
