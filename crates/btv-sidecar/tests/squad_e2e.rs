...
//!
//!   Rust CoreService (scripted)  ⇄  Python btv_squad.server (real)
//!
//! O Rust sobe o `CoreService`, spawna o servidor Python do squad de
//! verdade (`uv run python -m btv_squad.server`) apontado para o socket
...
use btv_proto::squad::{handoff, squad_event, SquadEvent, SquadTask};
use btv_sidecar::{drain_stream, serve_core, CoreBackend, SquadRun, SquadSupervisor};
...
// também a configuração real do `btv squad`.
...
async fn squad_python_real_streama_eventos_pelo_laco_bidirecional() {
    ...
    let core_sock = std::env::temp_dir().join(format!("btv-squad-core-{pid}.sock"));
    let squad_sock = std::env::temp_dir().join(format!("btv-squad-{pid}.sock"));
...
async fn kill_do_sidecar_dispara_fallback() {
    ...
    let pid = std::process::id();
    let core_sock = std::env::temp_dir().join(format!("btv-kill-core-{pid}.sock"));
    let squad_sock = std::env::temp_dir().join(format!("btv-kill-squad-{pid}.sock"));
...
/// `btv squad "crie X.html..."` produz `X.html` de verdade no
...
struct ScriptedCoreWithTools {
    tools: Arc<btv_tools::ToolRegistry>,
    permissions: btv_core::PermissionEngine,
    root: PathBuf,
    filename: String,
...
impl ScriptedCoreWithTools {
    /// Mesma lógica de `btv-cli::squad::core_run_tool` — duplicada aqui
    /// porque `btv-sidecar` (onde este teste mora) não pode depender de
    /// `btv-cli` (a direção de dependência é a oposta). Isolado o
    ...
    async fn run_tool_real(&self, call: &ToolCall) -> ToolResult {
        ...
        let allowed = match self.permissions.evaluate(&call.tool, &scope) {
            btv_core::Decision::Allow => true,
            btv_core::Decision::Deny => false,
            btv_core::Decision::Ask => true, // request_permission sempre aprova neste teste
        };
    ...
    fn log_tool_run(&self, call: &ToolCall, scope: &str, result: &ToolResult) {
        let dir = self.root.join(".btv");
        let _ = std::fs::create_dir_all(&dir);
        let Ok(mut store) = btv_store::LedgerStore::open(dir.join("btv.db").to_str().unwrap())
        else {
            return;
        };
        let _ = store.append(btv_schemas::ledger::LedgerEntry {
            seq: 0,
            prev_hash: String::new(),
            entry_hash: String::new(),
            kind: "squad.tool_run".into(),
            actor: "btv-cli:squad-tool".into(),
...
/// original — `btv squad "crie X.html..."` produzindo `X.html` real no
...
async fn squad_cria_arquivo_real_via_run_tool_ledger_e_auditor_veem_evidencia() {
    ...
    let backend = ScriptedCoreWithTools {
        tools: Arc::new(btv_tools::ToolRegistry::default_set(workspace_root.path())),
        permissions: btv_core::PermissionEngine::default(),
    ...
    let pid = std::process::id();
    let core_sock = std::env::temp_dir().join(format!("btv-squad-close-core-{pid}.sock"));
    let squad_sock = std::env::temp_dir().join(format!("btv-squad-close-{pid}.sock"));
    ...
    let ledger = btv_store::LedgerStore::open(
        workspace_root
            .path()
            .join(".btv")
            .join("btv.db")
    ...
    let entries = ledger
        .recent(50, Some("btv-cli:squad-tool"))
