//! Teste de integração cross-process real do comando `forge verify`: sobe o
//! binário compilado de verdade (`Command::new(env!("CARGO_BIN_EXE_forge"))`,
...
//! teste mediria o Forge real por acidente.
//!
...
fn write_forge_toml(dir: &Path, contents: &str) {
    std::fs::write(dir.join("forge.toml"), contents).unwrap();
}

fn only_evidence_file(dir: &Path) -> std::path::PathBuf {
    let evidence_dir = dir.join(".forge").join("evidence");
...
"#,
    ...
    let output = Command::new(forge_bin())
        ...
        .output()
        .expect("forge verify deveria rodar");
...
"#,
    ...
    // sem forge.toml na raiz do fixture, o default só seria usado se
...
#[test]
fn sem_forge_toml_cai_no_default_que_espelha_o_ci() {
    // Fixture sem forge.toml algum — deve cair no default_steps() (test+lint+fmt
    ...
    Command::new(forge_bin())
        ...
        .current_dir(dir.path())
        .output()
        .expect("forge verify deveria rodar mesmo sem forge.toml");
