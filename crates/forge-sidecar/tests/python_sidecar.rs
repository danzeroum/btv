...
//! verdade (`uv run python -m forge_promptforge.server`) via
...
use forge_sidecar::SidecarSupervisor;
...
async fn sidecar_python_real_responde_por_grpc() {
    ...
    let socket =
        std::env::temp_dir().join(format!("forge-sidecar-test-{}.sock", std::process::id()));
