...
Onda 8, ADR 0022. Mirror de `btv_promptforge.server`: sidecar stateless,
...
Uso: python -m btv_squad.memory_server --socket <memory.sock> [--memory-dir <dir>]
"""
...
from btv_proto import memory_pb2, memory_pb2_grpc
...
async def serve(socket_path: str, memory_dir: Optional[str] = None) -> None:
    ...
    await server.start()
    logger.info("btv_squad memory sidecar ouvindo em %s", socket_path)
