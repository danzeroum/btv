...
gera texto de modelo é sempre o gateway Rust (`forge-llm`). Se este sidecar
cair ou não subir, o `forge-sidecar` do lado Rust degrada graciosamente
(pula lint/geradores) em vez de falhar a tarefa do usuário.

Uso: python -m forge_promptforge.server --socket /caminho/para.sock
"""
...
from forge_proto import promptforge_pb2, promptforge_pb2_grpc
from forge_promptforge.generators import GENERATORS
...
async def serve(socket_path: str) -> None:
    ...
    await server.start()
    logger.info("forge_promptforge sidecar ouvindo em %s", socket_path)
