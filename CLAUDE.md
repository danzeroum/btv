...
- O hash de cache de prompt (`prompt-cache-key.v1`) tem implementação dupla:
  ...
  `python/packages/btv-promptforge/src/btv_promptforge/hashing.py` (Python).
...
- Ledger é append-only com hash-chain (`crates/btv-store/src/ledger.rs`) —
  nunca UPDATE/DELETE; overrides são novas entradas marcadas.

## BuildToValue (produto novo sobre o motor BuildToValue)
...
handoff §4) servido pelo `btv dashboard` como SPA **raiz** (o console
BuildToValue `web/` continua em `/dev`, com `base: './'`). Contrato novo
...
`AuditLedger` hash-encadeado espelhada no ledger BuildToValue (`btv.flow_saved`).
...
`btv-store::btv` (`.btv/btv.db` — runs, entregas, personas, publicação
...
rota mutável; sessão de código roda pelo navegador via SSE (`btv-cli::
...
validado", nunca finge aplicar ao squad real. `btv dashboard` roda com
...
`max_autonomy_level` e `btv_squad/forgetting.py` (código morto,
...
sandbox Docker real (`btv-tools::sandbox`, bollard, em Rust) confina os
...
`sandbox` do CI com Docker real). Um cliente MCP (`btv-tools::mcp`, `rmcp`)
expõe tools de servidores externos sob o mesmo motor de permissões (ADR 0012),
e um cliente LSP hand-rolled (`btv-tools::lsp`, zero-dep) dá definição/
...
(`btv_squad/recall.py`, ADR 0013). `btv experiment`
(`btv-schemas::experiment`, `experiment.v1`) gera o relatório de A/B da
...
(`btv-schemas`/`btv-core`/`btv-llm`) rodam no CI (job `bench`) e um
...
`/verify` (`crates/btv-verify`) roda um pipeline configurável
(timeout + kill de *grupo* de processos, parsers reais para
cargo test/clippy/ruff) e produz `verification-evidence.v1`; `btv
...
(fail-closed). `btv_review` pondera quatro reviewers, mas
...
registrável no ledger. O skill-vetter (`btv-verify::vetter`, ADR 0009)
...
`verify` do CI roda `btv verify` sobre o próprio workspace e falha o
build em veredito `Fail`.

**Fase 4 — squad multi-agente + gRPC bidirecional (ADRs 0004–0007):** o
sidecar Python `btv_squad` roda o `UnifiedOrchestrator` (5 agentes
...
Python só conhece o UDS. `btv squad` renderiza os eventos ao vivo,
...
`schemas/proto/promptforge.proto`; `btv-proto/build.rs` compila via
...
(grpcio-tools, não betterproto — mais maduro). `btv-sidecar`:
`SidecarSupervisor::spawn`+`wait_ready` sobe `uv run python -m
btv_promptforge.server` e espera o health check; `SidecarClient` fala
`PromptForgeService` sobre UDS. `btv-cli/src/sidecar.rs::try_start()`
...
Fechamento da Fase 3: `btv-llm::RateLimiter` (sliding window tier-gated,
decorator `RateLimitedGenerator` em `btv-cli/src/rate_limit_gen.rs`,
composto por baixo do `CachedGenerator` — hit de cache nunca consome vaga);
`btv-store::{Telemetry, PromptLibrary}` (offline-first, SQLite local,
falhas nunca derrubam o caminho principal); `/prompt
save|library|use|fav|rm` no chat (biblioteca de prompts); `btv-server`
(axum) + comando `btv dashboard` servindo `.btv/telemetry.db` num
...
- CI em `.github/workflows/ci.yml`: cargo test/clippy/fmt + pytest + gitleaks
  (bloqueante) + cargo-deny + job `verify` (self-hosting: `btv verify` sobre
